...
- `packages/ingestion/src/pkb_ingestion/` - the ingestion library, imported by the API. `pipeline.py` (orchestrates chunk -> embed -> build index batch), `chunker.py` (AST-based markdown chunker), `markdown_ast.py` (markdown-it-py block tree + source reconstruction), `tokens.py` (`TokenCounter` Protocol + heuristic token estimator), `embeddings.py` (`EmbeddingProvider` Protocol + `LocalEmbeddingProvider` (sentence-transformers/bge-small-zh-v1.5) + `OpenAICompatibleEmbeddingProvider`), `index_contracts.py` (Qdrant/OpenSearch payload builders), `ids.py` (`stable_chunk_id` + `qdrant_point_id`), `pdf_converter.py` (PyMuPDF PDF->markdown boundary; text-layer extraction only, scanned pages degrade gracefully).
...
`IngestionPipeline.build_index_batch(artifact)` produces an `IndexBatch` of three parallel lists - `chunk_records` (Postgres), `qdrant_points`, `opensearch_documents` - all keyed by the same stable chunk ID. The pipeline is pure data transformation; it does **not** write to any store. Persisting to Postgres/Qdrant/OpenSearch is done by `IngestionService` (`ingestion_service.py`), which the `POST /documents` endpoint calls. `IngestionService` accepts `.md`/`.markdown`/`.txt` directly (no converter) and `.pdf` via PyMuPDF; it is idempotent on re-ingest because all three stores key on the stable chunk ID (Qdrant by the derived UUID point ID).

`MarkdownChunker` parses markdown into a block AST (markdown-it-py) and walks it in document order. Headings update section metadata (H1 is the doc title and dropped; H2+ flush the buffer so a chunk never straddles sections); `<!-- page: N -->` markers set page metadata. Paragraphs, lists, blockquotes, code, and tables accumulate in a buffer up to the token budget (`max_tokens=512`, matching bge-small-zh-v1.5's max sequence length), then split at a node boundary. A single oversized node is split by the Sentence Splitter, with a hard Token Splitter fallback (with overlap) for any sentence still over budget. Token sizes come from an injectable `TokenCounter` (`HeuristicTokenCounter` by default: CJK ~1 token/char, Latin ~1/4); the same counter populates `Chunk.token_count` (was a raw character count). `LocalEmbeddingProvider` is the default - a real semantic embedding via sentence-transformers (bge-small-zh-v1.5, 512-dim, L2-normalized for Qdrant COSINE), loaded lazily on first use. An OpenAI-compatible API embedding is available via `OpenAICompatibleEmbeddingProvider`, selected by `make_embedding_provider(settings)` when `EMBEDDING_PROVIDER=openai` and the `EMBEDDING_API_*` keys are set; otherwise the local model is used. `EMBEDDING_DIMENSIONS` must match the model (512 for bge-small-zh).
