...
- `packages/ingestion/src/pkb_ingestion/` - the ingestion library, imported by the API. `pipeline.py` (orchestrates chunk -> embed -> build index batch), `chunker.py` (AST-based markdown chunker), `markdown_ast.py` (markdown-it-py block tree + source reconstruction), `tokens.py` (`TokenCounter` Protocol + heuristic token estimator), `embeddings.py` (`EmbeddingProvider` Protocol + `LocalEmbeddingProvider` (sentence-transformers/bge-small-zh-v1.5) + `OpenAICompatibleEmbeddingProvider`), `index_contracts.py` (Qdrant/OpenSearch payload builders), `ids.py` (`stable_chunk_id` + `qdrant_point_id`), `docling_converter.py` (PDF->markdown boundary).
